...
.sem-imagem .exportar [data-exportar="png"], .sem-imagem .exportar [data-exportar="jpeg"], .sem-imagem .exportar [data-exportar="pdf"],
.sem-imagem .exportar .grupo:first-child, .sem-excel .exportar [data-exportar="xlsx"] { display: none; }
.exportar .menu .grupo { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; color: var(--muted); padding: 6px 8px 2px; }
.exportar .menu .grupo + button { margin-top: 0; }
.exportar .menu button + .grupo { border-top: 1px solid var(--line); margin-top: 4px; }
...
const ferramentas = (id) => `
  <div class="gtools">
    <details class="exportar"><summary title="Exportar em alta resolução">Exportar</summary>
      <div class="menu">
        <span class="grupo">Imagem</span>
        ${["png", "jpeg", "pdf"].map((f) => `<button data-exportar="${f}" data-g="${id}">${f.toUpperCase()}</button>`).join("")}
        <span class="grupo">Dados</span>
        <button data-exportar="csv" data-g="${id}">CSV</button>
        <button data-exportar="xlsx" data-g="${id}">Excel</button>
        <button data-exportar="md" data-g="${id}">Markdown</button>
      </div>
    </details>
...
function renderGraficos(lista) {
  ...
  GRAFICOS = [
    { id: "artigos-por-ano", titulo: "Artigos por ano",
      ...
      tabela: { colunas: ["Ano", "Artigos"], linhas: anos.map((ano, k) => [ano, nArt[k]]) } },
    { id: "citacoes-por-ano", titulo: "Citações por ano de publicação",
      ...
      tabela: { colunas: ["Ano", "Citações", "Artigos", "Média de citações por artigo"],
                linhas: anos.map((ano, k) => [ano, nCit[k], nArt[k], Number((nCit[k] / nArt[k]).toFixed(2))]) } },
    { id: "como-encontrados", titulo: "Como os artigos foram encontrados",
      sub: "no Semantic Scholar", desenhar: () => barrasH(i3, "match"),
      tabela: { colunas: ["Método", "Artigos", "Percentual (%)"],
                linhas: i3.map((i) => [i.r, i.v, Number(((i.v / lMatch.length) * 100).toFixed(1))]) } },
    { id: "autores-por-artigo", titulo: "Número de autores por artigo",
      ...
      tabela: { colunas: ["Número de autores", "Artigos"], linhas: faixas.map((f, k) => [f === 8 ? "8 ou mais" : f, hist[k]]) } },
  ];
...
function filtrosAtivos() {
  const f = [];
  if (anosOff.size) {
    const marcados = TODOS_ANOS.filter((a) => !anosOff.has(a));
    f.push(`anos: ${marcados.length <= 6 ? marcados.join(", ") : `${marcados.length} de ${TODOS_ANOS.length} anos`}`);
  }
  ...
  [["soCitados", "só com citações"], ["soSimilares", "só similares"], ["soDuplicatas", "só com duplicata"],
   ["soTitulo", "só encontrados por título"], ["soErro", "só com erro ao coletar"]]
    .forEach(([id, rot]) => { if ($(id).checked) f.push(rot); });
...
function exportarDados(id, formato) {
  ...
  const filtros = `Filtros: ${filtrosAtivos()}`;
  const nome = `sbes-${id}`;

  if (formato === "csv") {
    const cel = (v) => { const t = String(v ?? ""); return /[",\n;]/.test(t) ? `"${t.replace(/"/g, '""')}"` : t; };
    const csv = [colunas, ...linhas].map((l) => l.map(cel).join(",")).join("\r\n");
    baixar("\ufeff" + csv + "\r\n", `${nome}.csv`, "text/csv;charset=utf-8");  // BOM: acentos corretos no Excel
  }
  if (formato === "md") {
  ...
  if (formato === "xlsx") {
    if (!window.XLSX) return alert("Exportação para Excel indisponível (sem conexão com o CDN).");
    const aoa = [[g.titulo], [g.sub], [filtros], [], colunas, ...linhas, [], [fonte]];
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws["!cols"] = colunas.map((c, k) => ({ wch: Math.max(c.length, ...linhas.map((l) => String(l[k]).length)) + 2 }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, g.titulo.slice(0, 31));
...
function cliqueGraficos(e) {
  ...
  if (exp) {
    ...
    const f = exp.dataset.exportar;
    if (["csv", "xlsx", "md"].includes(f)) exportarDados(exp.dataset.g, f);
...
<script>
  // sem as bibliotecas (sem internet), some só o que depende delas; CSV e Markdown funcionam sempre
  if (!window.html2canvas || !window.jspdf) document.body.classList.add("sem-imagem");
  if (!window.XLSX) document.body.classList.add("sem-excel");
</script>
<script>
